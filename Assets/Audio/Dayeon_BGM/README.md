...
| Stage_2_97BPM_Loop.wav | 스테이지2.wav 1.365–159.743 s | 97 | 256 | 7,600,825 | 0.34 | 80 ms OLA + 5 ms endpoint |
...
각 소스는 지정 구간을 목표 프레임 수로 위상 보존 리타이밍한 뒤 순환 경계를 보정했다. 강한 싱코페이션 때문에 자동 onset 검출만으로 악기 attack과 음악적 강박을 구분할 수 없어서, 신뢰할 수 없는 마디별 워핑은 출력에 적용하지 않았다. 최종 가청 판단은 복제 씬에서 메트로놈을 함께 재생해 시작·중간·마지막 마디와 강제 루프 경계를 확인한다.
